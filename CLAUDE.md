...
- **Method**: Sequence log-likelihood (P(prompt+"SAFE") vs P(prompt+"FAIL"))
- **Output**: Calibrated probability P(FAIL) ∈ [0,1]
- **Speed**: ~10-20ms per classification
...
Key settings:
- `stage1.backend: mlx` - Use MLX for true logit extraction
- `stage1.sequence_loglikelihood.enabled: true` - Use sequence log-likelihood
...
- All 128 tests should pass: `uv run pytest`
